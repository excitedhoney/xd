...
static const CGFloat MarginLeft = 6;
...
- (void)layoutSubviews
{
    ...
    if (!self.imageView) {
        ...
        NSLog(@"layoutSubviews pecropview =%@,%@",NSStringFromCGRect(self.insetRect),NSStringFromCGRect(self.bounds));
        [self setupImageView];
    }
...
- (void)setupImageView
{
    ...
    cropRect = CGRectMake(6, cropRect.origin.y, screenWidth - 2*6, cropRect.size.height);
    
    NSLog(@"layoutSubviews setupImageView =%@,%@",NSStringFromCGSize(self.image.size),NSStringFromCGRect(cropRect));
    ...
    NSLog(@"layoutSubviews setupImageView =%@,%@",NSStringFromCGRect(self.scrollView.bounds),NSStringFromCGRect(self.zoomingView.bounds));

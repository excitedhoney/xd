...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        cropBorderView = [[UIImageView alloc] initWithFrame:CGRectInset(self.bounds, -2.0f, -2.0f)];
        cropBorderView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
...
//        UIImage * pePhoto = [UIImage imageWithContentsOfFile: [bundle pathForResource:@"PEPhotoCropEditorBorder" ofType:@"png"]];
        cropBorderView.image = [[UIImage imageNamed:@"PEPhotoCropEditor.bundle/PEPhotoCropEditorBorder"] resizableImageWithCapInsets:UIEdgeInsetsMake(23.0f, 23.0f, 23.0f, 23.0f)];
//        imageView.image = pePhoto;
...
//    CGRect rect = CGRectMake(0, self.frame.origin.y, self.frame.size.width, self.frame.size.height);
...
//    NSLog(@"rect 000 =%@",NSStringFromCGRect(rect));

...
//    UIImage * newImage = [[self capture] mutableCopy];
    [self.screenShotsList addObject:[self capture]];
...
- (UIImage *)capture
{
//    return [UIImage imageWithColor:[UIColor colorWithRed:239.0/255 green:65.0/255 blue:86.0/255 alpha:1] size:self.view.frame.size] ;
...
//    @autoreleasepool {
        ...
        NSLog(@"IGraphicsGetCurrentContext() =%@",UIGraphicsGetCurrentContext());
        //    [self.view.layer performSelectorOnMainThread:@selector(renderInContext:) withObject:(__bridge id)(UIGraphicsGetCurrentContext()) waitUntilDone:YES];
...
- (void)paningGestureReceive:(UIPanGestureRecognizer *)recoginzer
{
    ...
    if (recoginzer.state == UIGestureRecognizerStateBegan) {
        ...
        if (touchPoint.x - startTouch.x > 110)
        {
...
//                bPopFromThis = YES;
                [SJKHEngine Instance]->bPopAnimate = NO;
